...
AppCircle.prototype = {
	...
	nearCoords: [[2,0], [2,2], [0,2], [-2,2], [-2, 0], [-2, -2], [0, -2], [2, -2], [-2,4], [0, 4], [2, 4] ],			//	nearCoords[11]
	getNearApps: function() {
		var nearAppList = [];
		for (var i = 0; i < 11; i++) {											//	6
			var nearCoord = Utility.sumCoord(this.coord, this.nearCoords[i]);
			//console.log("getNearApps ", i, nearCoord);
	...
	getNearAppsWithDir: function (dir) {
		...
		for (var i=1; i<nearApps.length;i++) {
			nearAppsWithDir.push(nearApps[i]);
		}
	...
	tunePosition: function(tuneValue) {
		...
		//console.log("tunePosition ", x - this.radius, y - this.radius);
	},
	...
	makeIcon: function(iconType) {
		...
		svg.setAttribute("viewBox", "-50 -50 100 100");				//	-50 -50 100 100
