function Display(_element, _map) {
	...
	this.maxScale = 100;
	this.minScale = 60;
	this.defaultRadius = 0.56;			//	0.125   0.46   !!!
}
...
Display.prototype = {
	...
	draw: function() {
		...
		console.log("Draw", this.currentCenter.getPosition());
	...
	setScale: function(newScale) {
		...
		var newViewBox = [centerPos[0] - this.width / this.scale / 2, 
	...
	clicked: function(target, pos) {
		...
		var viewBox = this._getViewBox();
		var clickedPos = [viewBox[0] + pos[0] / this.scale, 
		...
		if (clickedCircle === undefined  || clickedCircle.isSentinel()) {
	...
	getNearCircle: function(pos) {
		...
		for (var r = 0; r < 1; r += 0.05) {
			...
			if (guessCircle === undefined || guessCircle.isSentinel()) continue;
	...
	snapCenterTo: function(circle) {
		if (circle === undefined  || circle.isSentinel()) return;
	...
	snapAnimation: function(circle, timestamp) {
		...
		// 변화될 상태 계산
		//	Рассчитать государство быть изменены
		...
		if (progress < 1) {
			...
			if (circle != undefined) this.currentCenter = circle;
			this.interactionEnabled = true;
			animationTime = 0;
			// 클릭 이벤트리스너 복구
			//	Нажмите на восстановление прослушивателя событий
	...
	openApp: function(circle) {
		...
		var circlePos = circle.getPosition();
		var appWidth = (circle.iconType == IconType.ANIMATE) ? circle.radius * 2 : circle.radius * 2;		//	1.2
		var endViewBox = [circlePos[0] - appWidth/2, 
		...
		var animation = function(timestamp) {
			...
			// 처음과 끝이 정해져있는 애니메이션들
			//	Начало и конец анимации определяется, что
			...
			//Тогда анимация для выравнивания рассчитывается
	...
	_refreshCenter: function() {
		...
		if (circleAtCenter != undefined) this.currentCenter = circleAtCenter;
	...
	_resizeAllCircles: function() {
		for (var coord in this.map.apps) {
			...
			if (circle != undefined) {
				...
				if (this.currentCenterCandidates[0] != undefined && this.currentCenterCandidates[1] != undefined) {
					var zoomInRadius = this.triangleInterpolation(circle, this.currentCenter, 
																  this.currentCenterCandidates[0], 
																  this.currentCenterCandidates[1], 
																  this._getRadius.bind(this));
					var zoomOutRadius = this.triangleInterpolation(circle, this.currentCenter, 
																   this.currentCenterCandidates[0], 
																   this.currentCenterCandidates[1], 
	...
	_getZoomOutRadius: function(circle, psudoCenter) {
		...
		return Math.max(0.56 - 0.07 * dist, this.defaultRadius);		//	0.45
	},
	...
	_tunePositions: function() {
		for (var coord in this.map.apps) {
			var circle = this.getCircleAtCoord(coord);
			var circlePos = circle.getPosition();
			var zoomInTuneDistance = this.triangleInterpolation(circle, this.currentCenter, 
															this.currentCenterCandidates[0], 
															this.currentCenterCandidates[1], 
	...
	setCandidatesForCenter: function(newCenterCandidates) {
		if (newCenterCandidates!=undefined) this.currentCenterCandidates = newCenterCandidates;
	},
